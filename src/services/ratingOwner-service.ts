...
export class RatingOwnerService {
  //basePath = 'http://localhost:3000/Users';
  // basePath='https://server-sergio.herokuapp.com/';
  //basePath = 'http://localhost:8615/api/v1/rents';
  // basePath = 'http://localhost:8615/api/v1/ratingOwners';
  basePath = 'https://ratings-erentcar.azurewebsites.net/api/v1/ratingOwners';
  ...
  postRatingOwner(ownerId:number,clientId:number,data:any) {
    return this.http.post<any>(`${this.basePath}/owner/${ownerId}/client/${clientId}`, data, this.httpOptions)
      .pipe(
        retry(2),
        catchError(this.handleError));
  }
  getByOwner(owner:number){
    // return this.http.get<any>(`${this.basePath}/?id=${id}`, this.httpOptions)api/v1/
    return this.http.get<any>(`${this.basePath}/owner/${owner}`, this.httpOptions)
      .pipe(
        retry(2),
        catchError(this.handleError));
  }
